...
// intentionally tiny helpers; good enough for our input
static void chop_crlf(char* s){ size_t n=strlen(s); while(n && (s[n-1]=='\n' || s[n-1]=='\r')) s[--n]='\0'; }
static int split_tabs(char* line, char* out[], int max){ int k=0; char* p=line; while(*p && k<max){ while(*p=='\t' || *p=='\n' || *p=='\r') ++p; if(!*p) break; out[k++]=p; while(*p && *p!='\t' && *p!='\n' && *p!='\r') ++p; if(*p){ *p='\0'; ++p; } } return k; }
static char* sdup(const char* s){ size_t n=strlen(s); char* p=(char*)malloc(n+1); if(p) memcpy(p,s,n+1); return p; }
...
// duplicate id or OOM; free and bail
