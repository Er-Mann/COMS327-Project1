...
node_t* find_by_name(graph_t* G, const char* name){
    for(int i=0;i<G->nodeCount;i++){
        ...
        if(d->name && strcmp(d->name, name)==0) return n;
    }
    return NULL;
}
...
typedef enum { OP_LOC, OP_DIST, OP_DIAM, OP_ROAD } opk_t;
typedef struct { opk_t k; char* a; char* b; } op_t;

int main(int argc, char** argv){
    if(argc==1){ usage(); return 0; }

    const char* file = NULL;
    op_t ops[64]; int opn=0;
    ...
    if(!file){ usage(); return 0; }

    graph_t* G = NULL;
    if(!load_graph(file, &G)){ fprintf(stderr,"Could not load %s\n", file); return 1; }
    ...
    for(int i=0;i<opn;i++){
        ...
        if(op->k==OP_LOC){
            ...
            double best=-1; node_extra_t *A=NULL,*B=NULL;
            for(int u=0; u<G->nodeCount; u++){
                ...
                for(int v=u+1; v<G->nodeCount; v++){
                    ...
                    if(d>best){ best=d; A=du; B=dv; }
                }
            }
            if(A&&B) printf("%.5f %.5f %.5f %.5f %.2f\n",
                            (double)A->lat,(double)A->lon,(double)B->lat,(double)B->lon,best);
            else printf("0 0 0 0 0\n");
        }
        else if(op->k==OP_ROAD){
            node_t* s =find_by_name(G, op->a);
            node_t* t = find_by_name(G, op->b);
            if(!s || !t){ printf("NOTFOUND\n"); continue; }
            double d = dijkstra(G, s, t);
            if(d<0) printf("UNREACHABLE\n"); else printf("%.3f\n", d);
        }
    }
